...
export async function followAtLeastOne(follower_user_id){
    const {rows: [{check: isFollowed}]} = await db.query(`SELECT COUNT(id)>0 as check FROM follows WHERE follower_user_id = $1;`, [follower_user_id]);
    return isFollowed;
}